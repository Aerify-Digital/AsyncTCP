...
static void _async_service_task(void *pvParameters)
{
    ...
    const TickType_t xFrequency = CONFIG_ASYNC_TCP_TICK_SLEEP;
...
static bool _start_async_task()
{
    ...
    if (!_async_service_task_handle)
    {
        xTaskCreateUniversal(_async_service_task, "async_tcp", CONFIG_ASYNC_TCP_RUNNING_HEAP, NULL, CONFIG_ASYNC_TCP_RUNNING_PRIORITY, &_async_service_task_handle, CONFIG_ASYNC_TCP_RUNNING_CORE);
        if (!_async_service_task_handle)
        {
            return false;
        }
...
AsyncClient::AsyncClient(tcp_pcb *pcb)
    : _connect_cb(0), _connect_cb_arg(0), _discard_cb(0), _discard_cb_arg(0), _sent_cb(0), _sent_cb_arg(0), _error_cb(0), _error_cb_arg(0), _recv_cb(0), _recv_cb_arg(0), _pb_cb(0), _pb_cb_arg(0), _timeout_cb(0), _timeout_cb_arg(0), _pcb_busy(false), _pcb_sent_at(0), _ack_pcb(true), _rx_last_packet(0), _rx_since_timeout(0), _ack_timeout(ASYNC_MAX_ACK_TIME), _connect_port(0), _recv_pbuf_flags(0), prev(NULL), next(NULL)
{
    _pcb = pcb;
    _closed_slot = -1;
    if (_pcb)
